 ...
 * 目前支持mysql、oracle、sybase和postgresql的分页，其它数据库暂不支持。
 * 
 * @author Hankin
 */
@Deprecated
...
public class PageInterceptor implements Interceptor {
    ...
    private static String defaultDialect = "postgresql"; // 数据库类型(默认为mysql)
    ...
    private void setPageParameter(String sql, Connection connection,
        ...
        try {
            ...
            int totalPage = (totalCount + page.getPageSize() - 1)
                    / page.getPageSize();
    ...
    private String buildSortSql(PageParameter page) {
        StringBuilder sql = new StringBuilder();
        if (!page.getOrderBy().isEmpty()) {
    ...
    private String buildSortForOrcalSql(PageParameter page) {
        StringBuilder sql = new StringBuilder();
        if (!page.getOrderBy().isEmpty()) {
            if (page.getOrderJdbcType().equalsIgnoreCase(
                    JdbcType.VARCHAR.name())) {
                sql.append(" ORDER BY ").append(page.getOrderBy()).append(" ")
                        .append(page.getSort());

            } else if (page.getOrderJdbcType().equalsIgnoreCase(
                    JdbcType.INTEGER.name())
                    || page.getOrderJdbcType().equalsIgnoreCase(
                            JdbcType.NUMERIC.name())) {
                sql.append(" ORDER BY to_number(translate(")
                        .append(page.getOrderBy()).append(", '0123456789' || ")
                        .append(page.getOrderBy()).append(", '0123456789')) ")
                        .append(page.getSort());
            } else {
                sql.append(" ORDER BY ").append(page.getOrderBy()).append(" ")
     ...
     * @return
     */
    private StringBuilder buildPageSqlForPostgreSQL(String sql,
            PageParameter page) {
        // 计算第一条记录的位置，Mysql中记录的位置是从0开始的。
        String offset = String.valueOf((page.getCurrentPage() - 1)
                * page.getPageSize());
        StringBuilder pageSql = new StringBuilder();

        pageSql.append(sql);
        pageSql.append(buildSortSql(page));
        pageSql.append(" limit ").append(page.getPageSize()).append(" offset ")
                .append(offset);
        return pageSql;
    }
     ...
     * @return String
     */
    public static StringBuilder buildPageSqlForSybase(String sql,
            PageParameter page) {
        StringBuilder pageSql = new StringBuilder();
        ...
        if (beginrow * 2 > page.getTotalCount()) {
            if (fromSql.lastIndexOf("desc") > 0) {
                order = "asc";
                fromSql = fromSql.substring(0, fromSql.lastIndexOf("desc"))
                        + order;
            } else if (fromSql.lastIndexOf("asc") > 0) {
                order = "desc";
                fromSql = fromSql.substring(0, fromSql.lastIndexOf("asc"))
                        + order;
            }
            tempOrder = "desc";
        }

        pageSql.append("select top ").append(endrow)
                .append(" *,rownum=identity(int) into ").append(temp)
                .append(" ");
        pageSql.append(fromSql).append(" ");
        pageSql.append("select * from ").append(temp)
                .append(" where rownum > ").append(beginrow)
    ...
    public StringBuilder buildPageSqlForOracle(String sql, PageParameter page) {
        ...
        pageSql.append(sql);
        pageSql.append(buildSortForOrcalSql(page));
    ...
    public void setProperties(Properties properties) {
        PageInterceptor.dialect = properties.getProperty("dialect");
        PageInterceptor.pageSqlId = properties.getProperty("pageSqlId");
    }
}

package com.elane.core.aop.mybatis.mapper;
 ...
 * 方法，根据参数parameterObject配置的注解信息，自动生成sql语句。 签名里要拦截的类型只能是接口。
 * @author Hankin
 *
 */
@Deprecated
...
public class AutoMapperInterceptor implements Interceptor {
    ...
    private static String defaultCommonSelectSqlId = "commonSelect.*$"; // 需要拦截的ID(正则匹配)
    private static String commonSelectSqlId = ""; // 需要拦截的ID(正则匹配)
    ...
    public void setProperties(Properties properties) {
        AutoMapperInterceptor.commonSelectSqlId = properties.getProperty("commonSelectSqlId");
    }
